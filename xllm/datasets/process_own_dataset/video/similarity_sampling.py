...
from xllm.processors import Blip2ImageTrainProcessor
from xllm.models.xllm_models.blip2 import disabled_train
img_size = 224
drop_path_rate=0 
...
def run_video(video_path):
    ...
    visual_encoder = create_eva_vit_g(
        img_size, drop_path_rate, use_grad_checkpoint, precision='fp32'
    ).to(device).eval()
    ...
    image_process = Blip2ImageTrainProcessor(image_size=224)
    video_frames = get_clip_video_frames(video_path, image_process).to(device)

    with torch.no_grad():
        frames_fts = visual_encoder(video_frames).detach()
        frames_fts = torch.nn.functional.normalize(frames_fts, dim=-1).detach()
        frames_fts_ = frames_fts[:, 0, :]
        similiarities = frames_fts_ @ frames_fts_.T
